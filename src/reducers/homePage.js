...
//当组件第一次发出REQUEST_TOPIC后，需要对其返回的state进行初始化，否则没有topics等属性会报错
...
const homePage = (state={selectedTab: 'all',tabData: {}}, action) => {
    if(state) {
        const sTab = selectedTab(state.selectedTab, action); //选择tab页
        const tData = tabData(state.tabData,action);         //tab的数据
        //返回的一定是一个新的对象，如果只是改变原来的state，返回的还是原来的state对象，
        //就无法被store。subscribe监听到，从而不会对组件状态进行更新
